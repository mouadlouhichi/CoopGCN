...
* 📊 **Uncalibrated audit record:** Sparse LightGCN cells fail calibration by roughly 4–150×. Audit found symmetric graph messages were double-counted in checkpoint-generation code; a corrected aggregation helper is now used, but reruns are pending.
...
* 🧪 **Confirmatory workflow:** Schema-controlled pending records live in [`experiments/expected/`](experiments/expected/). [`scripts/build_confirmatory_artifacts.py`](scripts/build_confirmatory_artifacts.py) emits tables and SVG status figures without treating pending rows as results. Historical projected CSVs remain quarantined.
...
## Confirmatory CSVs, Tables, and Figures

The required calibration and experiment records are created before any new
claim is written:
...
Inputs are under `experiments/expected/`; generated LaTeX tables, SVG status
figures, and `experiments/ARTIFACT_STATUS.md` are explicit about pending rows.
Required records cover baseline calibration, multi-seed runs, frozen norm and
component ablations, neighbour-cap sweeps, proxy-vs-Shapley validation,
corruption, attribution, runtime, hyperedge footprint, and user-degree audits.
...
The retained common metrics are shown as **NDCG@20 / TR@20 / Coverage@20 (%)** strictly as an uncalibrated audit record. Machine-readable values are in [`data/measured_results.csv`](data/measured_results.csv); exact known settings and missing environment fields are in [`paper/retained_run_manifest.yaml`](paper/retained_run_manifest.yaml).
...
**Audit conclusion:** sparse baselines fail calibration, so this table is not comparative evidence. Frozen-norm/component reruns, calibrated baselines, multiple seeds, cap sweeps, proxy-vs-Shapley validation, and runtime remain pending.
